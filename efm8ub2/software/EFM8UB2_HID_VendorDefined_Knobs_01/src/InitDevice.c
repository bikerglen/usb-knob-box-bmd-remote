...
{
	// configure all port 0 pins as digital push-pull outputs and set them low
	// except P0.6 which is the LED which is set to open drain and high.
	P0MDIN  = 0xFF; // all digital not analog
	P0MDOUT = 0x00; // all open drain
	P0      = 0xFF; // all high

	// crossbar skips all port 0 pins
	P0SKIP  = 0xff;
}
...
{
	// P1.0 is UART TXD -- push pull
	// P1.1 is UART RXD -- open drain

	// configure all port 1 pins as digital push-pull outputs and set them low
	P1MDIN  = 0xFF; // all digital not analog
	P1MDOUT = 0x01; // all open drain except TXD
	P1      = 0xFF; // all high

	// crossbar skips all port 1 pins except P1.0 and P1.1
	P1SKIP  = 0xfc;
}
...
{
	// configure all port 2 pins as digital push-pull outputs
	// set unused bits low, /WR & /RD high, A9:A8 low
	P2MDIN  = 0xFF;  // all digital not analog
	P2MDOUT = 0x00;  // all open drain
	P2      = 0xFF;  // all high

	// crossbar skips all port 2 pins
	P2SKIP  = 0xff;
}
