...
extern volatile uint8_t xdata usbReportData[6];
	    ...
	    if (USBD_EpIsBusy(EP1IN) == false) {
	    	if (usbReportNeeded) {
			    ...
			    txBuffer[0] = 0x01;
  			    ...
  			    txBuffer[5] = usbReportData[4];
  			    txBuffer[6] = usbReportData[5];
				USBD_Write (EP1IN, txBuffer, 7, false);
	    	}
	    }
	}
}
                             ...
                             uint16_t remaining)
{
	...
	if (epAddr == EP1OUT) {
		if (rxBuffer[0] == 0x02) {
			if (rxBuffer[1] & 0x01) SW1_LED = LED_ON; else SW1_LED = LED_OFF;
			if (rxBuffer[1] & 0x02) SW2_LED = LED_ON; else SW2_LED = LED_OFF;
		}
		USBD_Read (EP1OUT, rxBuffer, 16, true);
	}
